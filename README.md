...
本机访问 `http://127.0.0.1:8080/`。目录应为已有 Git 仓库根目录；非仓库仍能浏览编辑文件。
...
- 生产环境用独立低权限账户，仅授予目标仓库权限；通过受保护的服务环境文件设置 PASSWORD，不使用 root 运行。
...
- 单用户、单工作区、单编辑缓冲；文本文件上限 1 MiB / 20,000 行，仅 UTF-8。二进制和其他编码不编辑。
